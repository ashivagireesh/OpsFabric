...
export default function BusinessNodePalette({ onClose }: BusinessNodePaletteProps) {
  const [search, setSearch] = useState('')
  const [expandedCats, setExpandedCats] = useState<Set<NodeCategory>>(
    new Set(['trigger', 'source', 'transform', 'destination', 'flow']),
  )
  const addNode = useBusinessWorkflowStore((s) => s.addNode)
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return ALL_BUSINESS_NODE_TYPES
    return ALL_BUSINESS_NODE_TYPES.filter((n) =>
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
        {categoryOrder.map((cat) => {
          ...
          const catInfo = BUSINESS_NODE_CATEGORIES[cat]
        ...
        {filtered.length === 0 && (
          <div style={{ padding: '32px 16px', textAlign: 'center' }}>
            <Text style={{ color: 'var(--app-text-dim)', fontSize: 13 }}>No business nodes match "{search}"</Text>
          </div>
        )}
      </div>
      ...
      >
        Click or drag into business canvas
      </div>
    </div>
  )
}
